...
/**/
func UpRelationShip(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		http.Error(writer, "An error occurred when trying to insert the relation!!! " + err.Error(), http.StatusBadRequest)
		return
	}
	if status == false {
		http.Error(writer, "The relationship could not be inserted!!! ", http.StatusBadRequest)
		return
	}
